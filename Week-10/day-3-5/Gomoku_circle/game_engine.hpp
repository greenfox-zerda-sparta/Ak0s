...
public:
  ...
  int* server_communication(int player, int x, int y) {
    IPaddress ip;
    ...
    int send_coordinates[3] = {player, x, y};
    SDLNet_TCP_Send(client, &send_coordinates, sizeof(send_coordinates));

    static int recieved_coordinates[3];
    SDLNet_TCP_Recv(client, &recieved_coordinates, 512);

    SDLNet_TCP_Close(client);
    SDLNet_FreeSocketSet(set);
    return recieved_coordinates;
  }
};
