...
void Graphics::draw_circle_and_cross(GameContext& context, bool clicked) {
  if (clicks % 2 == 0) {
    int* coordinates;
    coordinates = context.server_communication(1, 0, 0, clicks);
    map[coordinates[1] / 40][coordinates[2] / 40] = coordinates[0];
    clicks++;
  }
  else if (clicked and clicks % 2 == 1){
    map[context.get_mouse_coordinates().first / 40][context.get_mouse_coordinates().second / 40] = 2;
    context.server_communication(2, context.get_mouse_coordinates().first, context.get_mouse_coordinates().second, clicks);
    clicks++;
  }
}
