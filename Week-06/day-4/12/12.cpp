...
void increment_inner_element(vector<vector<int>>& _myvector, unsigned int index_of_inner_vector, unsigned int index_of_element);
...
// PRINT AFTER INCREMENTATION:
  increment_inner_element(outer_vector, 2, 0);
  for (unsigned int i = 0; i < outer_vector.size(); i++) {
    for (unsigned int j = 0; j < outer_vector[i].size(); j++) {
      cout << outer_vector[i][j] << " ";
    }
...
void increment_inner_element(vector<vector<int>>& _myvector, unsigned int index_of_inner_vector, unsigned int index_of_element) {
  _myvector[index_of_inner_vector][index_of_element]++;
}
