...
int main() {
  ...
  cout << "(Say y if the program asks the exact number you picked!)" << endl << endl;
  cout << "Is the number smaller than or equal to " << (min+max)/2 << "?" << endl;
  cin >> y_or_n;

  while (!done) {
	  if (max - min <= 2){
	    cout << "The number you picked is: " << max << endl;
	    done = true;
	    break;
	}
    else if (y_or_n == 'y') {
